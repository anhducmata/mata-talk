...
/// The main view model encapsulating root states and behaviors of the English learning app
/// such as connection, published tracks, conversation state, etc.
///
...
final class AppViewModel {
    ...
    enum Error: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .agentNotConnected:
                "English tutor did not connect to the session"
    ...
    private func getConnection() async throws -> TokenService.ConnectionDetails {
        ...
        guard let connectionDetails = try await tokenService.fetchConnectionDetails(
            ...
            throw Error.configurationError("LiveKit Sandbox ID not configured. Please set LIVEKIT_SANDBOX_ID in .env.xcconfig file.")
        }
